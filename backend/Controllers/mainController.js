...
const importData = async (req, res, next) => {
  try {
    const { professors, students, activities, feedback, participation } =
      req.body;
    const errors = [];
    ...
    else res.status(200).json({ message: 'Data imported successfully' });
  } catch (error) {
    next(error);
  }
};
...
export { createDatabase, importData, exportData };
